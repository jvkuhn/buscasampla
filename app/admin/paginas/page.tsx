...
import { deleteSitePage } from "@/lib/actions/settings";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Páginas — Admin" };

export default async function SitePagesPage() {
  const pages = await db.sitePage.findMany({ orderBy: { updatedAt: "desc" } });

  return (
    <div>
      <PageHeader
        title="Páginas institucionais"
        action={{ href: "/admin/paginas/nova", label: "+ Nova página" }}
      />
            ...
            {pages.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right space-x-3">
                  ...
                  <DeleteButton action={deleteSitePage} id={p.id} />
                </td>
              </tr>
            ))}
            {pages.length === 0 && (
